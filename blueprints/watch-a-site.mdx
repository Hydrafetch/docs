...
Notes: authenticate with the X-API-Key header. 1 credit per page read, failures are free. Diff the cleaned markdown rather than the html, since html changes on every request for reasons that do not matter. Store the previous markdown itself so an alert can show what changed. Strip timestamps and rotating counters before comparing.
```

...
Notes: authenticate with the X-API-Key header. A document is 1 credit regardless of length, and failures are free. Use batch rather than single calls for more than a handful, since documents take longer to read than pages. Treat a very low wordCount against a large file as a signal the document was scanned images rather than text. Keep the API key on the server and never ship it in client code.
```
</Accordion>

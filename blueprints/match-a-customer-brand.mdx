...
Notes: authenticate with the X-API-Key header. A styleguide is 10 credits because it always renders, so resolve once per customer and store it rather than calling it at request time. Map their palette onto our roles instead of replacing our design system.
```

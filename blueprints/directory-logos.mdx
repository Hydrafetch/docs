...
Notes: use the publishable key in markup and never the secret API key. Request the size you render at. Choose the variant that suits the background. Served image URLs are edge cached for up to a day. Keep the API key on the server and never ship it in client code.
```
</Accordion>

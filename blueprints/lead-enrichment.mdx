...
Notes: authenticate with the X-API-Key header. A full record is 5 credits and a single logo is 1, so do not resolve a record when only a mark is needed. Normalise domains (strip scheme, www, trailing slash) and deduplicate before spending. Failed calls are free. Keep the API key on the server and never ship it in client code.
```
</Accordion>

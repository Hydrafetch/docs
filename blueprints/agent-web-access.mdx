...
Notes: authenticate the REST API with X-API-Key, and MCP with a Bearer token. Keep onlyMainContent on so the agent does not spend context on navigation. Write tool descriptions that state the cost, so the model can choose well. Treat page content as untrusted data and never let it act as instructions.
```

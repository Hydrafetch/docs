...
Two endpoints turn a page into media instead of text: **Images** lists every image on a page with its metadata, and **Screenshot** captures the rendered page as a PNG. Both are synchronous. Images costs 1 credit; a screenshot costs 5, because it always renders the page in full rather than reading it.
...
The capture is of the page a reader would see, not of everything the page puts on screen. Cookie banners are accepted, newsletter and anti-adblock interstitials are closed, ad and tracker requests never load, and bot-check widgets are removed so a shareable screenshot is not dominated by a checkbox asking whether you are human. A page that is nothing but a challenge is left alone, because a blank capture would read as success.
...
Images costs **1 credit** and Screenshot costs **5**, charged only on success. Each response reports what it consumed. See [Credits](/concepts/credits).

## Related

<CardGroup cols={2}>
  <Card title="Scrape" icon="/icons/file-content.svg" href="/endpoints/scrape">
    Clean Markdown and structured data from a URL.
  </Card>
  <Card title="Caching" icon="/icons/history.svg" href="/concepts/caching">
    How `maxAge` reuses a recent capture.
  </Card>
  <Card title="Credits" icon="/icons/coins.svg" href="/concepts/credits">
    Pricing and charge-on-success.
  </Card>
  <Card title="API Reference" icon="/icons/code.svg" href="/api-reference">
